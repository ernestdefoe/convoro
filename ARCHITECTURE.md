...
> 1. **Composer path** — `ComposerRunner` locates a composer binary/phar and runs `composer require vendor/pkg` via a queued `ComposerTaskJob` (off-request, then `dump-autoload -o`). Composer-installed packages of `type: convoro-extension` are auto-discovered by `ExtensionManager` (from `vendor/composer/installed.json`) and autoload via Composer itself. Surfaced in the Marketplace only when Composer is detected. Best for dependency resolution + updates.
...
- **Cache invalidation (OPcache).** Production runs with PHP OPcache on, so adding/replacing extension PHP files must reset it or stale bytecode is served. Every lifecycle action (`installFromZip`/`Url`, `enable`, `disable`, `uninstall`) goes through `ExtensionInstaller` → `App\Support\Caches::bust()` (clear app+extension caches + `opcache_reset()`), which runs in the FPM request context so the reset is real. Extension authors don't reimplement this. Shell deploys reset OPcache by reloading php-fpm (`scripts/deploy.sh`); `opcache.validate_timestamps=1` is the 2s self-heal backstop. See deploy/DEPLOYMENT.md §4.
