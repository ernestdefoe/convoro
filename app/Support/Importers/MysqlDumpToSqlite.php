 ...
 * SQLite database. Forum migrations frequently arrive as a database FILE
 * (managed hosts that only hand you a dump) rather than a live connection —
 * once in SQLite, every existing importer works unchanged (they already run
 * against SQLite via the test fixtures).
 *
 * Scope: the common mysqldump shape (CREATE TABLE + extended INSERT). Only the
 * data matters for an import, so secondary keys/constraints are dropped and
 * column types collapse to SQLite affinities. MySQL string escapes (\' \\ \n …)
 * are re-escaped to SQLite form so text/quotes/newlines survive intact.
 */
class MysqlDumpToSqlite
{
    ...
    public static function convert(string $dumpPath, string $sqlitePath): array
    {
        ...
        foreach (self::statements($in) as $stmt) {
            ...
            // everything else (SET, LOCK/UNLOCK, DROP, /*! … */, comments) ignored
        }
        $pdo->commit();
        if (is_resource($in)) {
            gzclose($in);
    ...
    {
        $buf = '';
        $inStr = false; $inTick = false; $esc = false;
        ...
        while (($line = gzgets($fh)) !== false) {
            if ($buf === '') {
                ...
                if (str_starts_with($l, '/*')) {
                    // skip the conditional/version comment block
            ...
            for ($i = 0; $i < $len; $i++) {
                ...
                if ($inStr) {
                    if ($ch === '\\') { $esc = true; } elseif ($ch === "'") { $inStr = false; }
                ...
                if ($ch === "'") { $inStr = true; } elseif ($ch === '`') { $inTick = true; } elseif ($ch === ';') {
                    $s = trim($buf);
                    if ($s !== '') { yield $s; }
                    $buf = '';
                }
    ...
    private static function translateCreate(string $stmt): ?string
    {
        ...
        foreach (self::splitTop($body) as $part) {
            ...
            if ($part === '' || ! str_starts_with($part, '`')) {
                continue; // PRIMARY KEY / KEY / UNIQUE / CONSTRAINT / FOREIGN …
            }
            $end = strpos($part, '`', 1);
            if ($end === false) {
                continue;
    ...
    private static function translateInsert(string $stmt): string
    {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if (! $inStr) {
                if ($ch === '`') { $inTick = true; $out .= $ch; } elseif ($ch === "'") { $inStr = true; $out .= "'"; } else { $out .= $ch; }
            ...
            if ($ch === '\\' && $i + 1 < $len) {
                ...
                // could be the closing quote, or a doubled '' literal quote
                if ($i + 1 < $len && $stmt[$i + 1] === "'") { $out .= "''"; $i++; } else { $out .= "'"; $inStr = false; }
            } else {
                $out .= $ch;
            }
        }
    ...
    /** Return the substring inside the parens starting at $open (exclusive). */
    private static function balanced(string $s, int $open): ?string
    {
        $depth = 0; $inStr = false; $inTick = false; $esc = false; $start = $open + 1;
        for ($i = $open, $len = strlen($s); $i < $len; $i++) {
            $ch = $s[$i];
            if ($esc) { $esc = false; continue; }
            if ($inStr) { if ($ch === '\\') { $esc = true; } elseif ($ch === "'") { $inStr = false; } continue; }
            if ($inTick) { if ($ch === '`') { $inTick = false; } continue; }
            if ($ch === "'") { $inStr = true; } elseif ($ch === '`') { $inTick = true; } elseif ($ch === '(') { $depth++; } elseif ($ch === ')') {
    ...
    /** Split on top-level commas (ignoring commas inside parens/strings/idents). */
    private static function splitTop(string $s): array
    {
        $parts = []; $cur = ''; $depth = 0; $inStr = false; $inTick = false; $esc = false;
        for ($i = 0, $len = strlen($s); $i < $len; $i++) {
            $ch = $s[$i];
            if ($esc) { $cur .= $ch; $esc = false; continue; }
            if ($inStr) { $cur .= $ch; if ($ch === '\\') { $esc = true; } elseif ($ch === "'") { $inStr = false; } continue; }
            if ($inTick) { $cur .= $ch; if ($ch === '`') { $inTick = false; } continue; }
            if ($ch === "'") { $inStr = true; $cur .= $ch; } elseif ($ch === '`') { $inTick = true; $cur .= $ch; } elseif ($ch === '(') { $depth++; $cur .= $ch; } elseif ($ch === ')') { $depth--; $cur .= $ch; } elseif ($ch === ',' && $depth === 0) { $parts[] = $cur; $cur = ''; } else { $cur .= $ch; }
        }
