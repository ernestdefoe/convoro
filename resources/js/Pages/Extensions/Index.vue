...
function priceLabel(item: Item): string {
  if (!item.premium) return 'Free';
  const n = Number(item.price);
  return n > 0 ? `$${n}` : 'Premium';
}
</script>

<template>
  <Head title="Extensions & Themes" />
  <AppLayout>
    <div class="mx-auto max-w-[var(--c-container)]">
      <!-- Hero -->
      <header class="mb-6 rounded-c border border-line bg-surface p-7 sm:p-9">
        <h1 class="text-3xl font-extrabold tracking-tight sm:text-4xl">Extensions &amp; Themes</h1>
        <p class="mt-2 max-w-2xl text-ink-2">Extend your community — add features and reskin it with one click. Free and premium add-ons, all built for Convoro.</p>
      </header>
      ...
      <div class="mb-5 flex flex-wrap items-center gap-3">
        ...
        <div class="ml-auto flex items-center gap-2 rounded-full border border-line bg-surface px-4 py-2">
          ...
          <input v-model="query" type="search" placeholder="Search add-ons…" class="w-48 border-0 bg-transparent p-0 text-sm text-ink placeholder:text-ink-muted focus:ring-0" />
        </div>
      </div>
        ...
        <article v-for="item in visible" :key="item.id"
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="text-xs text-ink-muted">{{ item.author || 'Convoro' }} · v{{ item.version }}</p>
            </div>
          </div>
          ...
          <div class="mt-4 flex items-center gap-2 border-t border-line pt-4">
            ...
            <span v-if="item.active" class="rounded-full bg-emerald-500/15 px-2.5 py-0.5 text-xs font-semibold text-emerald-600">Active</span>
      ...
      <p class="mt-8 text-center text-sm text-ink-muted">
        More free &amp; premium add-ons are coming to the Convoro store. Built something? Developer docs are on the way.
      </p>
    </div>
  </AppLayout>
</template>
