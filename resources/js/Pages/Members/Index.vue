  ...
  <AppLayout>
    <div class="mx-auto max-w-[var(--c-container)]">
      <div class="mb-5 flex flex-wrap items-center gap-3">
        <div>
          <h1 class="text-2xl font-extrabold tracking-tight">Members</h1>
          <p class="text-sm text-ink-muted">{{ total }} {{ total === 1 ? 'member' : 'members' }}</p>
        </div>
        <div class="ml-auto flex items-center gap-2 rounded-full border border-line bg-surface px-4 py-2">
          ...
          <input v-model="query" @input="search" type="search" placeholder="Find a member…" class="w-44 border-0 bg-transparent p-0 text-sm text-ink placeholder:text-ink-muted focus:ring-0" />
        </div>
      ...
      <div v-if="members.next" class="py-6 text-center">
        <Link :href="members.next" class="rounded-c border border-line bg-surface px-5 py-2.5 text-sm font-semibold hover:bg-surface-2">Load more</Link>
      </div>
    </div>
  </AppLayout>
</template>
