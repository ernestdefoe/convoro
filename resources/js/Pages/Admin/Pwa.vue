  ...
  <AdminLayout>
    ...
    <div class="grid gap-6 lg:grid-cols-2">
      ...
      <section class="rounded-2xl border border-line bg-surface p-6">
        <h3 class="mb-4 text-sm font-bold text-ink">Install settings</h3>
        <form class="space-y-5" @submit.prevent="save">
          ...
          <label class="flex items-center gap-3">
            ...
            <span class="text-sm text-ink-2">Show the “install app” banner on supported devices</span>
          </label>
        ...
        <p class="mt-4 text-xs text-ink-muted">The theme color comes from the brand color in <strong>Theme</strong>. Push notifications use VAPID (configured server-side).</p>
      </section>

      <!-- Icons -->
      <section class="rounded-2xl border border-line bg-surface p-6">
        <h3 class="mb-1 text-sm font-bold text-ink">App icons</h3>
        <p class="mb-4 text-xs text-ink-muted">Upload one square image — every PWA/favicon size is generated automatically (WebP/PNG).</p>
        ...
        <div class="mt-4">
          ...
          <span v-if="iconForm.processing" class="ml-2 text-sm text-ink-2">Generating…</span>
        </div>
      </section>
    </div>
  </AdminLayout>
