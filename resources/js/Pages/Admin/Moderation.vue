...
function deletePost(id: number) {
  askConfirm('Delete post', 'This permanently removes the post. This cannot be undone.', () =>
...
const banForm = reactive({ reason: 'Spam', ban_ip: true, delete_posts: false });
function openBan(id: number, name?: string) {
  banForm.reason = 'Spam'; banForm.ban_ip = true; banForm.delete_posts = false;
  banModal.value = { id, name: name || 'this member' };
}
function confirmBan() {
...
function unbanIp(id: number) {
  askConfirm('Unban IP', 'Allow this IP address to access the community again?', () =>
  ...
  <AdminLayout>
    ...
    <div v-if="tab === 'reports'" class="space-y-3">
      ...
      <div v-for="r in reports" :key="r.id" class="rounded-2xl border border-line bg-surface p-5">
        <div class="flex items-start gap-3">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p class="mt-1 text-xs text-ink-2">
              by <strong class="text-ink-2">{{ r.subject?.author || 'unknown' }}</strong>
              ...
              <span v-if="r.subject?.ip" class="ml-2 rounded bg-surface-2 px-1.5 py-0.5 font-mono text-[11px] text-ink-2">IP {{ r.subject.ip }}</span>
            </p>
            <p class="mt-1 text-xs text-ink-muted">Reported by {{ r.reporter || 'a member' }} · {{ r.createdAt }}<span v-if="r.reason"> · “{{ r.reason }}”</span></p>
          </div>
        </div>
        <div class="mt-4 flex flex-wrap items-center gap-2 border-t border-line pt-3">
          ...
          <button v-if="r.subject?.authorId" @click="openBan(r.subject.authorId, r.subject.author)" class="rounded-lg bg-red-500/15 px-3 py-1.5 text-xs font-semibold text-red-300 hover:bg-red-500/25">Ban author</button>
          <button v-if="r.subject?.ip" @click="openBanIp(r.subject.ip)" class="rounded-lg bg-surface-2 px-3 py-1.5 text-xs font-semibold text-ink-2 hover:bg-surface-2">Ban IP</button>
          ...
          <button @click="dismiss(r)" class="rounded-lg px-3 py-1.5 text-xs font-semibold text-ink-2 hover:bg-surface-2">Dismiss</button>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="tab === 'users'" class="space-y-2">
      ...
      <div v-for="u in bannedUsers" :key="u.id" class="flex items-center gap-3 rounded-xl border border-line bg-surface p-4">
        ...
        <button v-if="u.lastIp" @click="openBanIp(u.lastIp)" class="rounded-lg bg-surface-2 px-2.5 py-1.5 text-xs font-semibold text-ink-2 hover:bg-surface-2">Ban IP</button>
    ...
    <div v-else class="space-y-4">
      <div class="rounded-2xl border border-line bg-surface p-5">
        <h2 class="mb-3 text-sm font-bold text-ink">Ban an IP address</h2>
        <div class="flex flex-wrap items-end gap-3">
          <div><label class="block text-xs text-ink-2">IP address</label><input v-model="newIp" placeholder="203.0.113.5" class="mt-1 w-44 rounded-lg border-line bg-appbg font-mono text-sm text-ink focus:border-indigo-500 focus:ring-indigo-500" /></div>
          <div class="flex-1"><label class="block text-xs text-ink-2">Reason (optional)</label><input v-model="newIpReason" class="mt-1 w-full rounded-lg border-line bg-appbg text-sm text-ink focus:border-indigo-500 focus:ring-indigo-500" /></div>
      ...
      <div v-if="!ipBans.length" class="rounded-2xl border border-dashed border-line p-10 text-center text-sm text-ink-2">No banned IPs.</div>
      <div v-for="b in ipBans" :key="b.id" class="flex items-center gap-3 rounded-xl border border-line bg-surface p-4">
        ...
        <button @click="unbanIp(b.id)" class="ml-auto rounded-lg px-2.5 py-1.5 text-xs font-semibold text-ink-2 hover:bg-emerald-500/10 hover:text-emerald-300">Unban</button>
      </div>
    </div>
    ...
    <div v-if="banModal" class="fixed inset-0 z-[80] flex items-center justify-center p-4" @keydown.esc="banModal = null">
      ...
      <div class="relative w-full max-w-md rounded-2xl border border-line bg-surface p-6 shadow-2xl">
        <h3 class="text-lg font-bold text-ink">Ban {{ banModal.name }}</h3>
        <p class="mt-1 text-sm text-ink-2">They'll be signed out and blocked from posting or registering.</p>
        <label class="mt-4 block text-xs font-semibold uppercase tracking-wide text-ink-2">Reason</label>
        ...
        <label class="mt-3 flex items-center gap-2.5 text-sm text-ink">
          ...
          Also ban their last known IP address
        </label>
        <label class="mt-2 flex items-center gap-2.5 text-sm text-ink">
          ...
          Also delete all their replies
        </label>
        <div class="mt-5 flex justify-end gap-2">
          <button @click="banModal = null" class="rounded-lg px-4 py-2 text-sm font-semibold text-ink-2 hover:bg-surface-2">Cancel</button>
          <button @click="confirmBan" class="rounded-lg bg-red-500 px-4 py-2 text-sm font-semibold text-white hover:bg-red-600">Ban member</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="ipModal" class="fixed inset-0 z-[80] flex items-center justify-center p-4" @keydown.esc="ipModal = null">
      ...
      <div class="relative w-full max-w-md rounded-2xl border border-line bg-surface p-6 shadow-2xl">
        <h3 class="text-lg font-bold text-ink">Ban IP <span class="font-mono text-indigo-300">{{ ipModal.ip }}</span></h3>
        <p class="mt-1 text-sm text-ink-2">Requests from this address will be blocked.</p>
        <label class="mt-4 block text-xs font-semibold uppercase tracking-wide text-ink-2">Reason (optional)</label>
        ...
        <div class="mt-5 flex justify-end gap-2">
          <button @click="ipModal = null" class="rounded-lg px-4 py-2 text-sm font-semibold text-ink-2 hover:bg-surface-2">Cancel</button>
          <button @click="confirmBanIp" class="rounded-lg bg-red-500 px-4 py-2 text-sm font-semibold text-white hover:bg-red-600">Ban IP</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="confirmModal" class="fixed inset-0 z-[80] flex items-center justify-center p-4" @keydown.esc="confirmModal = null">
      ...
      <div class="relative w-full max-w-sm rounded-2xl border border-line bg-surface p-6 shadow-2xl">
        ...
        <div class="mt-5 flex justify-end gap-2">
          <button @click="confirmModal = null" class="rounded-lg px-4 py-2 text-sm font-semibold text-ink-2 hover:bg-surface-2">Cancel</button>
          <button @click="runConfirm" class="rounded-lg px-4 py-2 text-sm font-semibold text-white" :class="confirmModal.danger ? 'bg-red-500 hover:bg-red-600' : 'bg-indigo-500 hover:bg-indigo-600'">Confirm</button>
        </div>
      </div>
    </div>
  </AdminLayout>
