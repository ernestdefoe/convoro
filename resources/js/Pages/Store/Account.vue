      ...
      <div v-else class="mt-8 space-y-3">
        <div v-for="l in licenses" :key="l.key" class="rounded-2xl border border-line bg-surface p-5">
          <div class="flex flex-wrap items-center gap-3">
            ...
            <span class="ml-auto text-xs text-ink-muted">Purchased {{ l.purchased }}</span>
          </div>
          <div class="mt-3 flex flex-wrap items-center gap-3">
            ...
            <a v-if="l.download" :href="l.download" class="rounded-lg border border-line px-3 py-2 text-sm font-semibold text-ink-2 hover:bg-surface-2">Download</a>
          </div>
        </div>
      </div>
    </section>
