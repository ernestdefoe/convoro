...
<template>
  <section class="overflow-hidden rounded-c border border-line bg-surface shadow-sm">
    <div class="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent" :class="compact ? 'p-4' : 'p-5'">
      ...
      <p v-if="!compact" class="mt-1 text-sm text-ink-muted">Get an instant answer from the community's knowledge — with links to the threads it came from.</p>

      <form class="mt-3" :class="compact ? 'flex flex-col gap-2' : 'flex gap-2'" @submit.prevent="ask">
        <input
          v-model="q"
          type="text"
          :placeholder="t('Ask anything about {site}…', { site: siteName })"
          class="rounded-c border-line bg-surface-2 text-ink placeholder:text-ink-muted focus:border-primary focus:ring-primary"
          :class="compact ? 'w-full text-sm' : 'flex-1'"
        />
        <button
          type="submit"
          :disabled="loading || q.trim().length < 3"
          class="rounded-c bg-primary text-sm font-semibold text-white shadow-lg shadow-primary/30 hover:bg-primary-600 disabled:opacity-50"
          :class="compact ? 'w-full px-4 py-2' : 'shrink-0 px-5 py-2.5'"
        >
    ...
    <div v-if="asked" class="border-t border-line" :class="compact ? 'p-4' : 'p-5'">
