<!DOCTYPE html>
{{--
    Convoro override of the Horizon dashboard layout.
    Changes vs the vendor copy:
      • Removed Horizon's own light/dark <scheme-toggler> — the dashboard now
        simply follows the forum's theme (localStorage 'convoro_theme', falling
        back to the theme.mode setting), so it never mismatches the rest of admin.
      • A small script disables the non-matching <style data-scheme> block.
    If Horizon's vendor layout changes in a future release, re-sync this file.
--}}
<html lang="en">
<head>
    <!-- Meta Information -->
    ...
    @php($cvTheme = \App\Support\Settings::get('theme.mode', 'light') === 'dark' ? 'dark' : 'light')
    <meta name="color-scheme" content="{{ $cvTheme }}">
    ...
    <script>
        // Match the forum's theme: enable the forum-chosen scheme stylesheet,
        // disable the other. No flash, no per-dashboard toggle.
        (function () {
            var fallback = @json($cvTheme);
            var theme = fallback;
            try { theme = localStorage.getItem('convoro_theme') || fallback; } catch (e) {}
            var dark = theme === 'dark';
            function apply() {
                document.querySelectorAll('style[data-scheme]').forEach(function (s) {
                    s.media = (s.getAttribute('data-scheme') === (dark ? 'dark' : 'light')) ? 'all' : 'not all';
                });
            }
            apply();
